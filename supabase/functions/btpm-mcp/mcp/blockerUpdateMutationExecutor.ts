// API-Q.10D2 — Caller-bound MCP Blocker-update adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Blocker update.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in
// API-Q.10D1, `public.mcp_v1_update_blocker`, via the accepted
// `updateMcpV1Blocker(...)` adapter, using a FRESH anon-key Supabase client
...
import type { ApiV1UpdateBlockerBody } from "../../_shared/btpm-api/routes/blockers.ts";
import {
...
/** Canonical Blocker identity path prefix, reusing the accepted parser. */
const BLOCKER_PATH_PREFIX = "/v1/blockers/";

/** Exact client options passed to the injected client factory. */
export interface McpUpdateBlockerClientOptions {
  readonly auth: {
...
export type McpUpdateBlockerClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpUpdateBlockerClientOptions,
) => unknown;

/** Caller-bound MCP Blocker-update executor. */
export type McpV1UpdateBlockerExecutor = (
  request: Request,
  blockerId: string,
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_update_blocker`,
 * bound to the given Supabase URL and anon key. A fresh client is constructed
 * per invocation and never reused.
 */
export function createMcpV1UpdateBlockerExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpUpdateBlockerClientFactory,
): McpV1UpdateBlockerExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1UpdateBlocker(
    request: Request,
    blockerId: string,
    body: ApiV1UpdateBlockerBody,
    executionContext: McpMutationExecutionContext,
    ...
    // Reuse the single canonical Blocker identity contract
    // (`/v1/blockers/{blockerId}`).
    ...
    const { blockerId: canonicalBlockerId } = parseApiV1BlockerUpdatePath(
      `${BLOCKER_PATH_PREFIX}${blockerId}`,
    );

    // Revalidate the canonical COMPLETE scalar desired state with the single
    // canonical parser. All five fields remain required.
    const canonicalBody = parseApiV1UpdateBlockerBody(body);
    ...
    return await updateMcpV1Blocker(client, {
      expectedOauthClientId: oauthClientId,
      blockerId: canonicalBlockerId,
