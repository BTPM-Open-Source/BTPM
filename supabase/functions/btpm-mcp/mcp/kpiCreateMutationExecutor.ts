// KPI-4C — Caller-bound MCP KPI-definition-create adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Project KPI definition creation.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in KPI-4A,
// `public.mcp_v1_create_kpi`, via the accepted `createMcpV1Kpi(...)` adapter,
...
import type { ApiV1CreateKpiBody } from "../../_shared/btpm-api/routes/kpis.ts";
import {
...
export interface McpCreateKpiClientOptions {
  readonly auth: {
...
export type McpCreateKpiClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpCreateKpiClientOptions,
) => unknown;

/** Caller-bound MCP KPI-create executor. */
export type McpV1CreateKpiExecutor = (
  request: Request,
  projectId: string,
...
/**
 * Revalidate the Project identity through the SINGLE accepted canonical
 * Project-KPI path authority (`parseApiV1ProjectKpisPath`). No second UUID
 * grammar is introduced here.
 */
function requireCanonicalProjectId(projectId: unknown): string {
  if (typeof projectId !== "string") internal();
  return parseApiV1ProjectKpisPath(`/v1/projects/${projectId}/kpis`).projectId;
}

/**
 * Create the caller-bound MCP executor for `public.mcp_v1_create_kpi`, bound
 * to the given Supabase URL and anon key. A fresh client is constructed per
 * invocation and never reused.
 */
export function createMcpV1CreateKpiExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpCreateKpiClientFactory,
): McpV1CreateKpiExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1CreateKpi(
    request: Request,
    projectId: string,
    body: ApiV1CreateKpiBody,
    executionContext: McpMutationExecutionContext,
    ...
    // Revalidate the canonical Project identity and the canonical business
    // body with the single canonical authorities.
    const canonicalProjectId = requireCanonicalProjectId(projectId);
    const canonicalBody = parseApiV1CreateKpiBody(body);
    ...
    return await createMcpV1Kpi(client, {
