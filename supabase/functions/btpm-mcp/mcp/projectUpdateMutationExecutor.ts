// API-Q Project Update Step 2 — Caller-bound MCP Project-update adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Project update.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in Project
// Update Step 1, `public.mcp_v1_update_project`, via the accepted
// `updateMcpV1Project(...)` adapter, using a FRESH anon-key Supabase client
...
//   - performs no authorization, mutation-gateway, capability, containment, Project
//     Connected-App enablement, persistence, provenance, encryption or audit
//     logic (the database is authoritative);
...
//   - does NOT re-parse the already-canonical Project update body through the
//     raw HTTP-body parser, and preserves every value + `set*` presence flag
//     exactly as supplied (absent and explicit-clear remain distinct);
...
import type { ApiV1UpdateProjectBody } from "../../_shared/btpm-api/routes/projects.ts";
import { parseApiV1ProjectUpdatePath } from "../../_shared/btpm-api/routes/projects.ts";
import {
...
/** Canonical Project identity path prefix used for reuse of the parser. */
const PROJECT_PATH_PREFIX = "/v1/projects/";

/** Exact client options passed to the injected client factory. */
export interface McpUpdateProjectClientOptions {
  readonly auth: {
...
export type McpUpdateProjectClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpUpdateProjectClientOptions,
) => unknown;

/** Caller-bound MCP Project-update executor. */
export type McpV1UpdateProjectExecutor = (
  request: Request,
  projectId: string,
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_update_project`,
 * bound to the given Supabase URL and anon key. A fresh client is constructed
 * per invocation and never reused.
 */
export function createMcpV1UpdateProjectExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpUpdateProjectClientFactory,
  ...
  return async function executeMcpV1UpdateProject(
    request: Request,
    projectId: string,
    body: ApiV1UpdateProjectBody,
    executionContext: McpMutationExecutionContext,
    ...
    // Reuse the single canonical Project identity contract
    // (`/v1/projects/{projectId}`). No Project row is read here.
    if (typeof projectId !== "string") {
      throw new ApiHttpError("invalid_request");
    }
    const { projectId: canonicalProjectId } = parseApiV1ProjectUpdatePath(
      `${PROJECT_PATH_PREFIX}${projectId}`,
    );

    // `body` is ALREADY the canonical parsed Project update desired state
    // (values plus derived `set*` presence flags). It must not be re-parsed
    // through the raw HTTP-body parser, which rejects derived keys. The shared
    // update adapter remains the validation / RPC-mapping boundary.
    ...
    return await updateMcpV1Project(client, {
