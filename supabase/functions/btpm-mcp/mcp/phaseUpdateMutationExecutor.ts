// API-Q Phase Update Step 2 — Caller-bound MCP Phase-update adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Phase update.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in Phase
// Update Step 1, `public.mcp_v1_update_phase`, via the accepted
// `updateMcpV1Phase(...)` adapter, using a FRESH anon-key Supabase client bound
...
import type { ApiV1UpdatePhaseBody } from "../../_shared/btpm-api/routes/phases.ts";
import {
...
/** Canonical Phase identity path prefix used for reuse of the accepted parser. */
const PHASE_PATH_PREFIX = "/v1/phases/";

/** Exact client options passed to the injected client factory. */
export interface McpUpdatePhaseClientOptions {
  readonly auth: {
...
export type McpUpdatePhaseClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpUpdatePhaseClientOptions,
) => unknown;

/** Caller-bound MCP Phase-update executor. */
export type McpV1UpdatePhaseExecutor = (
  request: Request,
  phaseId: string,
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_update_phase`, bound
 * to the given Supabase URL and anon key. A fresh client is constructed per
 * invocation and never reused.
 */
export function createMcpV1UpdatePhaseExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpUpdatePhaseClientFactory,
): McpV1UpdatePhaseExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1UpdatePhase(
    request: Request,
    phaseId: string,
    body: ApiV1UpdatePhaseBody,
    executionContext: McpMutationExecutionContext,
    ...
    return await updateMcpV1Phase(client, {
