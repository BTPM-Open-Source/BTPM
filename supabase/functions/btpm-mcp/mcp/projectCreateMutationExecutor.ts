// API-Q Project Create Step 2 — Caller-bound MCP Project-create adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Project creation.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in Project
// Create Step 1, `public.mcp_v1_create_project`, via the accepted
// `createMcpV1Project(...)` adapter, using a FRESH anon-key Supabase client
...
//     authoritative);
//   - queries no Project/Program/Workspace/business table;
//   - never queries, reads or writes api_project_client_enablements and never
//     auto-enables the created Project (Project Create has no target Project,
//     and the API-N.5 non-enablement rule is preserved);
...
import type { ApiV1CreateProjectBody } from "../../_shared/btpm-api/routes/projects.ts";
import { parseApiV1CreateProjectBody } from "../../_shared/btpm-api/routes/projects.ts";
import {
...
export interface McpCreateProjectClientOptions {
  readonly auth: {
...
export type McpCreateProjectClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpCreateProjectClientOptions,
) => unknown;

/** Caller-bound MCP Project-create executor. */
export type McpV1CreateProjectExecutor = (
  request: Request,
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_create_project`,
 * bound to the given Supabase URL and anon key. A fresh client is constructed
 * per invocation and never reused.
 */
export function createMcpV1CreateProjectExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpCreateProjectClientFactory,
): McpV1CreateProjectExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1CreateProject(
    request: Request,
    body: ApiV1CreateProjectBody,
    executionContext: McpMutationExecutionContext,
  ): Promise<ApiV1CreateProjectResult> {
    ...
    const canonicalBody = parseApiV1CreateProjectBody(body);
    ...
    return await createMcpV1Project(client, {
      expectedOauthClientId: oauthClientId,
      workspaceId: canonicalBody.workspaceId,
      name: canonicalBody.name,
      programId: canonicalBody.programId,
      deliveryModel: canonicalBody.deliveryModel,
