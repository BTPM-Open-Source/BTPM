// API-Q Program Update Step 2 — Caller-bound MCP Program-update adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Program updates.
//
// It invokes ONLY the fixed MCP-source database wrapper accepted in Program
// Update Step 1, `public.mcp_v1_update_program`, via the accepted
// `updateMcpV1Program(...)` adapter, using a FRESH anon-key Supabase client
...
import type { ApiV1UpdateProgramBody } from "../../_shared/btpm-api/routes/programs.ts";
import { parseApiV1ProgramUpdatePath } from "../../_shared/btpm-api/routes/programs.ts";
import {
...
/** Canonical Program identity path prefix used for reuse of the parser. */
...
export type McpUpdateProgramClientFactory = (
  ...
  options: McpUpdateProgramClientOptions,
) => unknown;

/** Caller-bound MCP Program-update executor. */
...
function isRpcClient(
  value: unknown,
): value is ApiV1ProgramUpdateMutationRpcClient {
  return (
    typeof value === "object" &&
    value !== null &&
    !Array.isArray(value) &&
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_update_program`,
...
export function createMcpV1UpdateProgramExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpUpdateProgramClientFactory,
  ...
  return async function executeMcpV1UpdateProgram(
    request: Request,
    programId: string,
    body: ApiV1UpdateProgramBody,
    executionContext: McpMutationExecutionContext,
    ...
    // Reuse the single canonical Program identity contract
    // (`/v1/programs/{programId}`). No Program row is read here.
    if (typeof programId !== "string") {
      throw new ApiHttpError("invalid_request");
    }
    const { programId: canonicalProgramId } = parseApiV1ProgramUpdatePath(
      `${PROGRAM_PATH_PREFIX}${programId}`,
    );

    // `body` is ALREADY the canonical parsed Program update desired state
    // (values plus the derived `setDescription` presence flag). It must not be
    // re-parsed through the raw HTTP-body parser, which rejects derived keys.
    // The shared update adapter remains the validation / RPC-mapping boundary.
    ...
    return await updateMcpV1Program(client, {
