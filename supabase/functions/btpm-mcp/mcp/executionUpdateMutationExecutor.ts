// API-Q.9A5 — Caller-bound MCP Execution Update adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for the first MCP mutation (`execution_updates.append`).
//
// It invokes ONLY the fixed MCP-source database wrapper created in API-Q.9A4,
// `public.mcp_v1_append_execution_update`, via the accepted
// `appendMcpV1ExecutionUpdate(...)` adapter, using a FRESH anon-key Supabase
...
//     persistence, provenance or audit logic (the database is authoritative);
//   - computes no payload hash and no idempotency decision;
//   - processes no confirmation (owned by `requireMcpMutationConfirmation`);
...
import type { ApiV1AppendExecutionUpdateBody } from "../../_shared/btpm-api/routes/executionUpdates.ts";
import { parseApiV1AppendExecutionUpdateBody } from "../../_shared/btpm-api/routes/executionUpdates.ts";
import {
...
export interface McpAppendExecutionUpdateClientOptions {
  readonly auth: {
...
export type McpAppendExecutionUpdateClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpAppendExecutionUpdateClientOptions,
) => unknown;

/** Caller-bound MCP execution-update executor. */
export type McpV1AppendExecutionUpdateExecutor = (
  request: Request,
 ...
 * `public.mcp_v1_append_execution_update`, bound to the given Supabase URL and
 * anon key. A fresh client is constructed per invocation and never reused.
 */
export function createMcpV1AppendExecutionUpdateExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpAppendExecutionUpdateClientFactory,
): McpV1AppendExecutionUpdateExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1AppendExecutionUpdate(
    request: Request,
    body: ApiV1AppendExecutionUpdateBody,
    executionContext: McpMutationExecutionContext,
  ): Promise<ApiV1AppendExecutionUpdateResult> {
    ...
    const canonicalBody = parseApiV1AppendExecutionUpdateBody(body);
