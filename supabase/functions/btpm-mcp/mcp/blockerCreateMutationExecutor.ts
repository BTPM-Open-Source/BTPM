// API-Q.10C2 — Caller-bound MCP Blocker-create adapter.
//
// This module provides exactly one factory that returns a caller-bound
// executor for MCP Blocker creation.
//
// It invokes ONLY the fixed MCP-source database wrapper created in API-Q.10C1,
// `public.mcp_v1_create_blocker`, via the accepted `createMcpV1Blocker(...)`
...
import type { ApiV1CreateBlockerBody } from "../../_shared/btpm-api/routes/blockers.ts";
import { parseApiV1CreateBlockerBody } from "../../_shared/btpm-api/routes/blockers.ts";
import {
...
export interface McpCreateBlockerClientOptions {
  readonly auth: {
...
export type McpCreateBlockerClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: McpCreateBlockerClientOptions,
) => unknown;

/** Caller-bound MCP Blocker-create executor. */
export type McpV1CreateBlockerExecutor = (
  request: Request,
...
/**
 * Create the caller-bound MCP executor for `public.mcp_v1_create_blocker`,
 * bound to the given Supabase URL and anon key. A fresh client is constructed
 * per invocation and never reused.
 */
export function createMcpV1CreateBlockerExecutor(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: McpCreateBlockerClientFactory,
): McpV1CreateBlockerExecutor {
  if (!isNonBlank(supabaseUrl)) internal();
  if (!isNonBlank(supabaseAnonKey)) internal();
  if (typeof createClient !== "function") internal();

  return async function executeMcpV1CreateBlocker(
    request: Request,
    body: ApiV1CreateBlockerBody,
    executionContext: McpMutationExecutionContext,
  ): Promise<ApiV1CreateBlockerResult> {
    ...
    const canonicalBody = parseApiV1CreateBlockerBody(body);
    ...
    return await createMcpV1Blocker(client, {
