// Phase 4D.14A.7E — Generate Decision Case PowerPoint one-pager (.pptx)
// and publish via the effective Organization-aware Tenant SharePoint +
// Microsoft Graph integrations.

import { createClient } from "https://esm.sh/@supabase/supabase-js@2.50.0";
import {
  mapDecisionCaseToOnepagerData,
  onepagerFilenameFor,
  MapError,
} from "./dataMapper.ts";
import { buildDecisionOnepagerBuffer } from "./decisionOnepagerDeckBuilder.ts";
import {
...
const FUNCTION_NAME = "generate-decision-case-ppt-onepager";
  ...
  try {
    ...
    if (authorized !== true) {
      return json(403, {
        ...
        note: "You do not have authority to generate the PowerPoint Decision Brief for this decision case.",
      });
    }
    ...
    if (!overwriteExisting) {
      const { data: existing } = await adminClient
        ...
        .eq("document_type", "decision_case_ppt_onepager")
      ...
      if (existing) {
        return json(409, {
          ok: false,
          error: "existing_onepager_conflict",
          note: "A Decision Brief one-pager already exists for this decision case. Confirm overwrite to regenerate.",
          existing: {
    ...
    let mapping;
    try {
      mapping = await mapDecisionCaseToOnepagerData(supabase, adminClient, recordId, userData.user.id);
    } catch (e) {
    ...
    const { data: deckData, snapshotAt, workspaceId, packageVersionNumber } = mapping;
    ...
    const filename = onepagerFilenameFor(deckData.projectName, deckData.decisionCase.title);
    let bytes: Uint8Array;
    try {
      bytes = await buildDecisionOnepagerBuffer(deckData);
    } catch (e) {
      const note = (e as Error).message || "Deck generation failed.";
      await supabase.rpc("record_generated_decision_case_document", {
        _record_id: recordId,
        _document_type: "decision_case_ppt_onepager",
    ...
    const sessionRes = await createTenantSharePointPublishSession({
      ...
      reason: "decision-case-ppt-onepager-sharepoint-publish",
      requestId,
    });
    ...
    if (!sessionRes.ok) {
      ...
      if (!targetRes.ok) {
        ...
        const upl = await publishGeneratedDocumentBytes({
          ...
          operation: "publish_decision_case_ppt_onepager",
        });
        if (upl.ok) {
    ...
    return json(200, {
      ...
      outcome_included: deckData.hasOutcome,
    });
  } catch (e) {
