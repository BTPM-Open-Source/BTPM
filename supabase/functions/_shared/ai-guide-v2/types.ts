// AI-GUIDE.V2.0 — Server-side mirror of V2 contracts.
// Kept in shared/ so all V2 modules and the V2 edge function can import them
// without touching the V1 runtime (supabase/functions/ai-help-chat).
//
// NOTE: This file intentionally duplicates the shape from src/types/aiGuideV2.ts
// to avoid cross-importing client code into edge runtime. Keep in sync.
...
export interface GuideV2KnowledgePackArticle {
  ...
  // V2.5-FIX: visibility-resolved KC excerpt (summary/tooltip) used for
  // grounding concept and troubleshooting answers. Never raw chunk text.
  summary?: string | null;
  // V2.6-FIX: visibility-resolved KC body excerpt (truncated) used to
  // enrich grounding snippets for concept/troubleshooting answers. Same
  // RLS path as the KC drawer; never raw vector chunk text.
  body_excerpt?: string | null;
}

export interface GuideV2GroundingSnippet {
  article_id: string;
  title: string;
  slug: string;
  snippet: string;
...
export interface GuideV2AnswerPlan {
  ...
  // V2.5-FIX: optional KC-grounded snippets for concept/troubleshooting modes.
  grounding_snippets?: GuideV2GroundingSnippet[];
  // ARCH.1D — Conceptual domain guidance points used by the renderer in
  // unverified safe-limit modes when the playbook for a diagnosed BTPM
  // situation supplies useful business obligations. These are NOT
  // click-by-click UI steps and MUST NOT be rendered as numbered steps.
  safe_guidance_points?: string[];
  // QA.4 — Concept-answer shape obligations. Renderer uses these to shape
  // helpful BTPM concept/help answers. Never final prose, never a substitute
  // for verified workflow steps.
...
// AI-GUIDE.V2-ARCH.1A — Domain Diagnosis contract (server mirror).
...
// AI-GUIDE.V2-ARCH.1E — Evidence-aware intent arbitration result (server mirror).
...
// AI-GUIDE.V2-STABILIZE.2 — Canonical Effective Decision (server mirror of
// the contract defined in effective-decision.ts). Downstream layers consume
// this object as the single authoritative source of truth.
export interface GuideV2EffectiveDecisionMirror {
