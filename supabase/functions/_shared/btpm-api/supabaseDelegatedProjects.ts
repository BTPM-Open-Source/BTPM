// API-H.3D — Caller-scoped `/v1/projects` read client factory.
//
// Binds the current request bearer token to a fresh anon-key Supabase
// client and invokes the accepted `readApiV1Projects` adapter with the
...
import {
  readApiV1Projects,
  type ApiV1ProjectsPayload,
  type ApiV1ProjectsQuery,
  type ApiV1ProjectsRpcClient,
} from "./supabaseProjects.ts";

/** Exact client options passed to the injected client factory. */
export interface DelegatedProjectsClientOptions {
  readonly auth: {
...
export type DelegatedProjectsClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: DelegatedProjectsClientOptions,
) => unknown;

/** Caller-scoped `/v1/projects` reader. */
export type DelegatedApiV1ProjectsReader = (
  request: Request,
...
function isRpcClient(value: unknown): value is ApiV1ProjectsRpcClient {
  return (
    typeof value === "object" &&
    value !== null &&
    !Array.isArray(value) &&
...
/**
 * Create a caller-scoped `/v1/projects` reader bound to the given
 * Supabase URL and anon key. A fresh client is constructed per
 * invocation.
 */
export function createDelegatedApiV1ProjectsReader(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: DelegatedProjectsClientFactory,
  ...
  return async function readDelegatedApiV1Projects(
    request: Request,
    context: AuthenticatedApiContext,
    query: ApiV1ProjectsQuery,
  ): Promise<ApiV1ProjectsPayload> {
    ...
    return await readApiV1Projects(client, oauthClientId, query);
  };
}
