// API-M.4 — Caller-scoped Project-planning read client factory.
//
// Binds the current request bearer token to a fresh anon-key Supabase
// client and invokes the accepted `readApiV1ProjectPlanning` adapter with
...
import {
  readApiV1ProjectPlanning,
  type ApiV1ProjectPlanningPayload,
  type ApiV1ProjectPlanningRpcClient,
} from "./supabaseProjectPlanning.ts";

/** Exact client options passed to the injected client factory. */
export interface DelegatedProjectPlanningClientOptions {
  readonly auth: {
...
export type DelegatedProjectPlanningClientFactory = (
  supabaseUrl: string,
  supabaseAnonKey: string,
  options: DelegatedProjectPlanningClientOptions,
) => unknown;

/** Caller-scoped Project-planning reader. */
export type DelegatedApiV1ProjectPlanningReader = (
  request: Request,
...
function isRpcClient(value: unknown): value is ApiV1ProjectPlanningRpcClient {
  return (
    typeof value === "object" &&
    value !== null &&
    !Array.isArray(value) &&
...
/**
 * Create a caller-scoped Project-planning reader bound to the given
 * Supabase URL and anon key. A fresh client is constructed per invocation.
 */
export function createDelegatedApiV1ProjectPlanningReader(
  supabaseUrl: string,
  supabaseAnonKey: string,
  createClient: DelegatedProjectPlanningClientFactory,
  ...
  return async function readDelegatedApiV1ProjectPlanning(
    request: Request,
    context: AuthenticatedApiContext,
    projectId: string,
  ): Promise<ApiV1ProjectPlanningPayload> {
    ...
    return await readApiV1ProjectPlanning(
      client,
      oauthClientId,
      projectId,
    );
