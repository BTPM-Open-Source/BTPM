...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-e-3b2-direct-table-read-containment_static_test.ts', import.meta.url).href;
// API-E.3B2 — restrictive OAuth direct-read containment (Category A tables 32–62).
//
// Static, repository-only contract test. Verifies that the single API-E.3B2
...
// each of entries 32..62 of the frozen API-E.3A Category A allowlist, and
// to no other table. Proves the migration performs no DDL besides those 31
// CREATE POLICY statements and no DML/data mutation. Also proves that the
// API-E.3B1 entries (1..31) are not modified by this step.
//
...
// Category A entries 32..62 — the exact tables covered by API-E.3B2.
...
// Category A entries 1..31 — the API-E.3B1 tables that MUST NOT appear in
...
async function loadMigration(): Promise<Migration> {
  ...
  const hits = all.filter((m) => /API-E\.3B2\b/.test(m.text));
  assertEquals(
    hits.length,
    1,
    `expected exactly one API-E.3B2 migration; got ${hits.length}: ${hits.map((h) => h.name).join(", ")}`,
  );
  return hits[0];
}

// ---------------------------------------------------------------------------
// 1. Allowlist coverage — this step covers exactly Category A entries 32..62.
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — covers exactly entries 32–62 of the frozen API-E.3A Category A allowlist",
  async () => {
    ...
    assertEquals(
      ...
      "API-E.3B2 EXCLUDED_TABLES must equal doc allowlist entries 1..31",
    );
    assertEquals(
      EXPECTED_TABLES as readonly string[],
      rest as readonly string[],
      "API-E.3B2 EXPECTED_TABLES must equal doc allowlist entries 32..62",
    );
...
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — migration creates the exact 31 restrictive authenticated SELECT policies with the exact API-E.1 condition",
  async () => {
    ...
    for (const table of EXPECTED_TABLES) {
      ...
      assert(
        listed,
        `API-E.3B2: table '${table}' missing from migration table array`,
      );
    }
    ...
    assertEquals(
      ...
      1,
      "API-E.3B2: expected exactly one CREATE POLICY format template in migration",
    );
    // No API-E.3B1 table may be named.
    for (const table of EXCLUDED_TABLES) {
      assert(
        !new RegExp(`'${table}'`).test(mig.text),
        `API-E.3B2: excluded (API-E.3B1) table '${table}' must not appear in migration`,
      );
    }
    // Exact literal condition must appear in source verbatim.
    assert(
      mig.text.includes(CONDITION),
      `API-E.3B2: exact USING condition string missing from migration`,
    );
    // Deterministic policy name literal must appear.
    assert(
      mig.text.includes(POLICY_NAME),
      `API-E.3B2: policy name literal missing from migration`,
    );
  },
);
...
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — migration alters no existing policy, table, function, trigger, or grant, and performs no DML",
  async () => {
    const mig = await loadMigration();
    ...
    for (const re of forbidden) {
      assert(
        !re.test(scrubbed),
        `API-E.3B2 migration must not contain SQL matching ${re}`,
      );
    }
    ...
    assertEquals(
      literalCreatePolicy,
      null,
      "API-E.3B2: no literal per-table CREATE POLICY statements permitted (must go through format(...) template)",
    );
  },
);

// ---------------------------------------------------------------------------
// 4. API-E.3B1 migration is unchanged by this step (entries 1..31 untouched).
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — does not modify Category A entries 1..31 (API-E.3B1 scope)",
  async () => {
    const mig = await loadMigration();
    assert(
      !/API-E\.3B1\b/.test(mig.text),
      "API-E.3B2 migration must not reference or modify API-E.3B1 artifacts",
    );
    for (const table of EXCLUDED_TABLES) {
      assert(
        !new RegExp(`\\b${table}\\b`).test(mig.text),
        `API-E.3B2 migration must not touch API-E.3B1 table '${table}'`,
      );
    }
  },
);
...
// ---------------------------------------------------------------------------
// 5. Truth-table semantics of the literal USING condition.
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — literal USING condition yields the intended containment truth table",
  () => {
    const evalCond = (
...
// ---------------------------------------------------------------------------
// 6. Self-audit — this static test performs no runtime database change.
// ---------------------------------------------------------------------------
Deno.test(
  "API-E.3B2 — this static test performs no runtime database change",
  async () => {
    ...
    for (const re of banned) {
      assert(
        !re.test(self),
        `API-E.3B2 test must not contain executable SQL matching ${re}`,
      );
    }
  },
);
