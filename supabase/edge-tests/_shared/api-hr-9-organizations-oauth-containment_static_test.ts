...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-9-organizations-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.9 — public.organizations OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.9 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.9");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.9 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.9");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.9");
});

Deno.test("API-HR.9 creates exactly one policy: api_e_oauth_read_containment on public.organizations", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.organizations\b/i.test(sql),
    "Policy must target public.organizations.",
  );
});

Deno.test("API-HR.9 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.9 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.9 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
    [...sql.matchAll(/\bpublic\.([a-z_][a-z0-9_]*)/gi)].map((m) => m[1].toLowerCase()),
  );
  assertEquals([...refs], ["organizations"], "Only public.organizations may be referenced.");
});

Deno.test("API-HR.9 leaves existing Organization policies untouched", async () => {
  const sql = await migrationSql();
  for (const name of ["org_select_members", "org_insert_authenticated", "org_update_admin"]) {
...
Deno.test("API-HR.9 leaves authority helpers and API-v1 wrappers untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "is_org_admin",
...
Deno.test("API-HR.9 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.9 changes no grant, function, trigger, index, view, enum or table definition", async () => {
  const sql = await migrationSql();
