// API-Q Phase Plan Step 1 — static contract guard for the trusted MCP
// database bridge.
//
...
const MARKER = "API-Q Phase Plan Step 1 — Trusted MCP Database Bridge";
...
async function loadMigration(): Promise<{ name: string; text: string }> {
  ...
  assert(found.length >= 1, "expected at least one Phase Plan bridge migration");
...
const EXECUTOR_SIGNATURE =
  "api_e_private.execute_v1_plan_phase(text, text, uuid, timestamptz, date, date, boolean, text, text, text, text)";
const WRAPPER_SIGNATURE =
  "(text, uuid, timestamptz, date, date, boolean, text, text, text, text)";

// A — exactly four function definitions.
Deno.test("Phase Plan bridge: exactly four functions are (re)defined", () => {
  ...
  assertEquals(
    ...
    new Set([
      ...
      "api_e_private.execute_v1_plan_phase",
      "public.api_v1_plan_phase",
      "public.mcp_v1_plan_phase",
    ]),
  );
});

// B — canonical overload signature unchanged.
Deno.test("Phase Plan bridge: canonical 5-argument overload keeps its signature", () => {
  assert(
...
Deno.test("Phase Plan bridge: the 4-argument UI overload is never recreated", () => {
  assert(
    !/CREATE OR REPLACE FUNCTION public\.apply_phase_planning_change\(_phase_id uuid, _new_start date/
      .test(sql),
    "the BTPM UI/internal overload must not be redefined",
  );
  assert(
    !/apply_phase_planning_change\(uuid, date, date, boolean\)/.test(sql),
    "no grant/revoke/comment may target the 4-argument overload",
  );
});

// D — trusted source allowlist is exactly external_api|mcp.
Deno.test("Phase Plan bridge: trusted source allowlist is exactly external_api and mcp", () => {
  assert(
...
Deno.test("Phase Plan bridge: no caller-supplied source argument exists", () => {
  const canonical = /CREATE OR REPLACE FUNCTION public\.apply_phase_planning_change\(([^)]*)\)/
    .exec(sql);
  ...
  for (const fn of ["api_v1_plan_phase", "mcp_v1_plan_phase"]) {
...
Deno.test("Phase Plan bridge: capability identity is hardcoded", () => {
  assert(sql.includes("c_api_version    constant text := 'v1';"));
  assert(sql.includes("c_capability_kind constant text := 'command';"));
  assert(sql.includes("c_capability_key constant text := 'phases:plan';"));
  assert(sql.includes("<> 'phases:plan'"), "exact capability containment must remain");
});

// G — untrusted source fails before Phase lookup/lock/write.
Deno.test("Phase Plan bridge: untrusted source fails before Phase lookup", () => {
  const canonicalStart = sql.indexOf(
    "CREATE OR REPLACE FUNCTION public.apply_phase_planning_change(",
  );
  ...
  const phaseLookup = body.indexOf("FROM public.phases");
  const lock = body.indexOf("FOR UPDATE");
  const write = body.indexOf("UPDATE public.phases");
  assert(guard > 0, "guard must exist");
  assert(guard < phaseLookup, "guard must precede the Phase lookup");
...
Deno.test("Phase Plan bridge: canonical planning semantics remain present", () => {
  for (const fragment of [
    "public.is_active_user(v_actor)",
    "public.is_workspace_admin_or_higher(v_actor, v_phase.workspace_id)",
    "expected_updated_at_required",
    "FOR UPDATE",
    "v_phase.updated_at IS DISTINCT FROM _expected_updated_at",
    "'stale_phase_planning'",
    "phase_completed_locked",
    "phase_cancelled_or_archived",
    "invalid_range",
    "'no_change'::public.pmg_command_status",
    "extend_project_window_required",
    "public._apply_project_extension_internal(",
...
Deno.test("Phase Plan bridge: no PMG command audit is introduced", () => {
  assert(
...
Deno.test("Phase Plan bridge: private executor is source-fixed and canonical", () => {
  ...
  const argsMatch = /CREATE OR REPLACE FUNCTION api_e_private\.execute_v1_plan_phase\(\s*([^,]+),/
    .exec(body);
  ...
  const calls = body.match(/public\.apply_phase_planning_change\(/g) ?? [];
...
// K — thin wrappers.
Deno.test("Phase Plan bridge: public wrappers are thin and source-fixed", () => {
  assert(
    /public\.api_v1_plan_phase\([\s\S]*?\$function\$\s*BEGIN\s*RETURN api_e_private\.execute_v1_plan_phase\(\s*'external_api',/
      .test(sql),
    "REST wrapper must delegate with fixed 'external_api'",
  );
  assert(
    /public\.mcp_v1_plan_phase\([\s\S]*?\$function\$\s*BEGIN\s*RETURN api_e_private\.execute_v1_plan_phase\(\s*'mcp',/
      .test(sql),
    "MCP wrapper must delegate with fixed 'mcp'",
  );
  const rest = /CREATE OR REPLACE FUNCTION public\.api_v1_plan_phase\(([\s\S]*?)\)\s*RETURNS jsonb/
    .exec(sql)?.[1] ?? "";
  const mcp = /CREATE OR REPLACE FUNCTION public\.mcp_v1_plan_phase\(([\s\S]*?)\)\s*RETURNS jsonb/
    .exec(sql)?.[1] ?? "";
...
// L — ACL.
Deno.test("Phase Plan bridge: private executor is not callable by any app role", () => {
  ...
  assert(
    !/GRANT EXECUTE ON FUNCTION api_e_private\.execute_v1_plan_phase/.test(sql),
    "the private executor must never be granted",
  );
});

Deno.test("Phase Plan bridge: wrapper grants are authenticated-only", () => {
...
Deno.test("Phase Plan bridge: no dynamic SQL or generic dispatch", () => {
  for (const forbidden of [
    "EXECUTE format",
    "EXECUTE '",
    "quote_ident",
...
Deno.test("Phase Plan bridge: no unrelated surface is touched", () => {
  for (const forbidden of [
    "CREATE POLICY",
    "ALTER TABLE",
    "DROP FUNCTION",
...
// N — registry exposure (owned by Phase Plan Step 4).
Deno.test("Phase Plan bridge: phases.plan is exposed as btpm_plan_phase", async () => {
  const registry = await Deno.readTextFile(
    new URL("../../functions/btpm-mcp/mcp/toolRegistry.ts", import.meta.url),
  );
  const idx = registry.indexOf('operationId: "phases.plan"');
  assert(idx > 0, "phases.plan must exist in the registry");
  const entry = registry.slice(idx, idx + 600);
  assert(entry.includes('exposure: "exposed"'));
  assert(entry.includes('btpm_plan_phase'));
  assert(
    !registry.includes("mcp_v1_plan_phase"),
    "registry must not reference the MCP wrapper",
  );
});

// O — the MCP wrapper name stays inside the accepted adapter only.
Deno.test("Phase Plan bridge: the MCP wrapper name never leaks into factory or runtime", async () => {
  for (
    const path of [
    ...
    assert(
      !text.includes("mcp_v1_plan_phase"),
      "the fixed MCP wrapper name must stay inside the accepted adapter",
    );
  }
});
