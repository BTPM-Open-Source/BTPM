...
const __BTPM_SRC_BASE__ = new URL('../../../../functions/_shared/btpm-api/__tests__/supabaseDelegatedProjects.test.ts', import.meta.url).href;
// API-H.3D — Focused tests for the caller-scoped `/v1/projects` reader.
...
import { createDelegatedApiV1ProjectsReader } from "../../../../functions/_shared/btpm-api/supabaseDelegatedProjects.ts";
...
import type { ApiV1ProjectsQuery } from "../../../../functions/_shared/btpm-api/supabaseProjects.ts";
...
function makeRequest(authorization?: string): Request {
  ...
  return new Request("https://api.example.com/v1/projects", { headers });
}
...
Deno.test("valid factory creation returns a reader function", () => {
  const read = createDelegatedApiV1ProjectsReader(
    URL_VALUE,
    ANON_KEY,
...
Deno.test("happy path: exact factory args, bearer, oauth client id, rpc args, frozen payload, fresh client per call", async () => {
  ...
  const read = createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, factory);
  ...
  assertEquals(rpcCalls[0].fn, "api_v1_list_projects");
...
Deno.test("factory-creation validation: url, anon key, and client factory", () => {
  ...
  for (const bad of [null, undefined, "factory", 5, {}]) {
    let err: unknown;
    try {
      createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, bad as never);
    } catch (caught) {
      err = caught;
    }
    assert(err instanceof ApiHttpError);
...
Deno.test("non-Request input and invalid context fail before client construction", async () => {
  let invoked = 0;
  const read = createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, () => {
    invoked += 1;
...
Deno.test("throwing or malformed client factory maps to internal_error", async () => {
  const throwing = createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, () => {
  ...
  for (const created of [null, undefined, [], "client", 5, { notRpc: true }]) {
    const malformed = createDelegatedApiV1ProjectsReader(
      URL_VALUE,
      ANON_KEY,
      () => created,
    );
...
Deno.test("project adapter errors propagate safely", async () => {
  ...
  const rejecting = createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, () => ({
  ...
  // Adapter-level query validation is not duplicated here; malformed
  // queries surface as adapter errors.
  const badQueryRead = createDelegatedApiV1ProjectsReader(URL_VALUE, ANON_KEY, () => ({
    rpc: () => Promise.resolve(makeOkPayload()),
  }));
  const badQueryErr = await assertRejects(
    () =>
      badQueryRead(
        makeRequest(`Bearer ${TOKEN}`),
        makeContext(),
        { workspaceId: "not-a-uuid", limit: 25, offset: 0, search: null } as never,
      ),
    ApiHttpError,
  );
  assertEquals(badQueryErr.code, "invalid_request");
});
...
Deno.test("module contains no environment, service-role, direct SDK, fetch, logging, caching, timer or generic RPC behavior", async () => {
  const source = await Deno.readTextFile(
    new URL("../supabaseDelegatedProjects.ts", __BTPM_SRC_BASE__),
  );
  for (
    const forbidden of [
      "Deno.env",
  ...
  assertEquals(source.split("readApiV1Projects(").length - 1, 1);
