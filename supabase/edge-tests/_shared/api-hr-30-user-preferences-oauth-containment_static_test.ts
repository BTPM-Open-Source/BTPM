...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-30-user-preferences-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.30 — User preference OAuth direct-read containment
// (parameterized static contract test).
//
// Structural inspection of the committed API-HR.30A/B migrations only. No live calls.
...
const SUB_STEPS: SubStep[] = [
  {
    id: "API-HR.30A",
    marker: "API-HR.30A",
    table: "user_active_context_preferences",
    protectedNames: [
      "Users read own active context prefs",
      "Users insert own active context prefs",
      "Users update own active context prefs",
      "Users delete own active context prefs",
      ...AUTHORITY_HELPERS,
      ...ENCRYPTION_HELPERS,
      "passwordResetOrganizationResolver",
      "touch_user_active_context_preferences",
      "trg_user_active_context_preferences_touch",
  ...
  },
  {
    ...
    protectedNames: [
      ...
      ...AUTHORITY_HELPERS,
      ...ENCRYPTION_HELPERS,
      "tg_user_saved_views_touch",
      "user_saved_views_touch",
      "surface_key",
      "scope_key",
      "state_payload",
...
for (const step of SUB_STEPS) {
  ...
  Deno.test(`${step.id} leaves existing ordinary policies and protected objects untouched`, async () => {
    const sql = await migrationSql(step.marker);
...
Deno.test("API-HR.30 sub-step migrations are two distinct files", async () => {
