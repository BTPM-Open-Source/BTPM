// API-Q Phase Plan Step 3 — focused guard for the Phase-planning MCP mutation
...
//
// Scope: control/composition only. `phases.plan` exposure is owned by Step 4 and
// unwired in this step.
...
import {
  createMcpPhasePlanToolExecutor,
  MCP_PHASE_PLAN_TOOL_ARGUMENT_NAMES,
  MCP_PHASE_PLAN_TOOL_ERROR_MESSAGES,
  MCP_PHASE_PLAN_TOOL_INPUT_SCHEMA,
  MCP_PHASE_PLAN_TOOL_NAME,
} from "../../functions/btpm-mcp/mcp/phasePlanMutationTool.ts";
...
const TOOL_URL = new URL(
  "../../functions/btpm-mcp/mcp/phasePlanMutationTool.ts",
  import.meta.url,
);
...
function validArgs(overrides: Record<string, unknown> = {}) {
  return {
    ...
    confirmation: true,
    idempotencyKey: "idem-key-phase-plan",
    ...overrides,
  };
}
...
function buildHarness(
  ...
  const executor = createMcpPhasePlanToolExecutor({
    request,
...
// ---------------------------------------------------------------------------
// A/B. Tool envelope and strict schema
// ---------------------------------------------------------------------------

Deno.test("A: tool name is exactly btpm_plan_phase and matches the registry", () => {
  assertEquals(MCP_PHASE_PLAN_TOOL_NAME, "btpm_plan_phase");
  const entry = MCP_TOOL_REGISTRY.find((e) => e.operationId === "phases.plan");
  assert(entry, "phases.plan registry entry missing");
  assertEquals(entry.toolName, MCP_PHASE_PLAN_TOOL_NAME);
});

Deno.test("B: exactly the seven approved argument names, in canonical order", () => {
  assertEquals([...MCP_PHASE_PLAN_TOOL_ARGUMENT_NAMES], [
    "phaseId",
    "expectedUpdatedAt",
    "startDate",
    "targetEndDate",
  ...
  assertEquals(
    Object.keys(MCP_PHASE_PLAN_TOOL_INPUT_SCHEMA.shape).sort(),
    [...MCP_PHASE_PLAN_TOOL_ARGUMENT_NAMES].sort(),
  );
  assert(MCP_PHASE_PLAN_TOOL_INPUT_SCHEMA.safeParse(validArgs()).success);
  for (const key of MCP_PHASE_PLAN_TOOL_ARGUMENT_NAMES) {
    const partial: Record<string, unknown> = validArgs();
    delete partial[key];
    assertFalse(
      MCP_PHASE_PLAN_TOOL_INPUT_SCHEMA.safeParse(partial).success,
...
Deno.test("C: unknown scope/provenance/control fields are rejected", () => {
  for (
    ...
    assertFalse(
      MCP_PHASE_PLAN_TOOL_INPUT_SCHEMA.safeParse(validArgs({ [key]: "x" }))
        .success,
...
// ---------------------------------------------------------------------------
// F/G/H. Canonical contract reuse
// ---------------------------------------------------------------------------

Deno.test("F/G/H: canonical Phase path, body and idempotency-payload builders are reused", () => {
  assert(toolSource.includes("parseApiV1PhasePlanningPath"));
  assert(toolSource.includes('"/v1/phases/"'));
  assert(toolSource.includes('"/planning"'));
  assert(toolSource.includes("parseApiV1PlanPhaseBody"));
  assert(toolSource.includes("buildApiV1PlanPhaseIdempotencyPayload"));
...
Deno.test("F2: invalid phaseId maps to invalid_arguments before the writer", async () => {
  const { executor, recorder } = buildHarness();
  const result = await executor(
    // deno-lint-ignore no-explicit-any
...
Deno.test("G2: canonically invalid business dates map to invalid_arguments", async () => {
  const { executor, recorder } = buildHarness();
  const result = await executor(
    // deno-lint-ignore no-explicit-any
    validArgs({ startDate: "2026-10-15", targetEndDate: "2026-09-01" }) as any,
  );
  assertEquals(result, { ok: false, category: "invalid_arguments" });
  assertEquals(recorder.writerCalls.length, 0);
});

Deno.test("I: canonical payload hash covers business fields only", async () => {
  const { executor, recorder } = buildHarness();
  assert((await executor(validArgs() as never)).ok);
  const canonicalBody = parseApiV1PlanPhaseBody({
    expectedUpdatedAt: EXPECTED_AT,
    startDate: "2026-09-01",
    targetEndDate: "2026-10-15",
    confirmParentExtension: false,
  ...
  const expectedHash = await hashCanonicalPayload(
    buildApiV1PlanPhaseIdempotencyPayload(PHASE_ID, canonicalBody),
  );
...
Deno.test("K: rate profile and consumption use exactly PHASE_PLANNING_ROUTE.id", async () => {
  const { executor, recorder } = buildHarness();
  assert((await executor(validArgs() as never)).ok);
  assertEquals(PHASE_PLANNING_ROUTE.id, "phases.plan");
  assertEquals(recorder.profileCalls, [
    { clientId: API_CLIENT_ID, routeId: "phases.plan" },
  ]);
  ...
  assertEquals(recorder.consumeCalls[0].routeId, "phases.plan");
});
...
Deno.test("L/M/N/O: writer invoked exactly once with canonical inputs forwarded unchanged", async () => {
  ...
  assertEquals(call.body.targetEndDate, "2026-10-15");
...
Deno.test("Q2: no Phase narrative field is ever returned", async () => {
...
Deno.test("R/S: extend_project_window_required maps to a distinct bounded category", async () => {
  ...
  // No retry after the confirmation-required outcome.
  assertEquals(recorder.writerCalls.length, 1);
  // The canonical code itself is not disclosed as a category.
  assertFalse(JSON.stringify(result).includes("extend_project_window_required"));
  assert(
    MCP_PHASE_PLAN_TOOL_ERROR_MESSAGES.project_window_extension_required
      .includes("new idempotency key"),
  );
});

Deno.test("T: stale planning maps to stale_phase_planning without currentUpdatedAt", async () => {
  ...
  assert(
    MCP_PHASE_PLAN_TOOL_ERROR_MESSAGES.stale_phase_planning.includes(
      "new idempotency key",
    ),
  );
});
...
// ---------------------------------------------------------------------------
// W/X/Y/Z/AA. Static architecture guards
// ---------------------------------------------------------------------------
...
Deno.test("Z: phases.plan is exposed as btpm_plan_phase (Step 4)", () => {
  const entry = MCP_TOOL_REGISTRY.find((e) => e.operationId === "phases.plan");
  assert(entry, "phases.plan registry entry missing");
