...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-20-ai-feature-settings-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.20 — public.ai_feature_settings OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.20 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.20");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.20 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.20");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.20");
});

Deno.test("API-HR.20 creates exactly one policy: api_e_oauth_read_containment on public.ai_feature_settings", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.ai_feature_settings\b/i.test(sql),
    "Policy must target public.ai_feature_settings.",
  );
});

Deno.test("API-HR.20 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.20 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.20 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
  ...
  assertEquals(
    [...refs],
    ["ai_feature_settings"],
    "Only public.ai_feature_settings may be referenced.",
  );
});

Deno.test("API-HR.20 leaves the existing org-admin read policy untouched", async () => {
  const sql = await migrationSql();
  assert(
    !sql.includes("Org admins can read AI feature settings"),
...
Deno.test("API-HR.20 leaves authority helper functions untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "is_org_admin",
...
Deno.test("API-HR.20 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.20 changes no grant, function, trigger, index, view, enum, constraint or table definition", async () => {
  const sql = await migrationSql();
