...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-11-tenant-memberships-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.11 — public.tenant_memberships OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.11 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.11");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.11 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.11");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.11");
});

Deno.test("API-HR.11 creates exactly one policy: api_e_oauth_read_containment on public.tenant_memberships", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.tenant_memberships\b/i
      .test(sql),
    "Policy must target public.tenant_memberships.",
  );
});

Deno.test("API-HR.11 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.11 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.11 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
  ...
  assertEquals(
    [...refs],
    ["tenant_memberships"],
    "Only public.tenant_memberships may be referenced.",
  );
});

Deno.test("API-HR.11 leaves the three existing Tenant-membership policies untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "tm_self_select",
    "tm_tenant_admin_select",
    "tm_tenant_admin_manage",
  ]) {
...
Deno.test("API-HR.11 leaves Tenant authority helpers and API-v1 wrappers untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "is_tenant_member",
    "is_tenant_owner",
...
Deno.test("API-HR.11 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.11 changes no grant, function, trigger, index, view, enum or table definition", async () => {
  const sql = await migrationSql();
