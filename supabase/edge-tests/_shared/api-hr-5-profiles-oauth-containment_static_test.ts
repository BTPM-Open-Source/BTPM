...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-5-profiles-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.5 — public.profiles OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.5 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.5");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.5 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.5");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.5");
});

Deno.test("API-HR.5 creates exactly one policy: api_e_oauth_read_containment on public.profiles", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.profiles\b/i.test(sql),
    "Policy must target public.profiles.",
  );
});

Deno.test("API-HR.5 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.5 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.5 targets no second table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
    [...sql.matchAll(/\bpublic\.([a-z_][a-z0-9_]*)/gi)].map((m) => m[1].toLowerCase()),
  );
  assertEquals([...refs], ["profiles"], "Only public.profiles may be referenced.");
});

Deno.test("API-HR.5 leaves profiles_select_same_org and can_read_profile untouched", async () => {
  const sql = await migrationSql();
  for (const name of ["profiles_select_same_org", "can_read_profile"]) {
...
Deno.test("API-HR.5 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.5 changes no grant, function, trigger, index, view or table definition", async () => {
  const sql = await migrationSql();
