...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-4-project-memberships-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.4 — project_memberships OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.4 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.4");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.4 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.4");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.4");
});

Deno.test("API-HR.4 creates exactly one policy: api_e_oauth_read_containment on public.project_memberships", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.project_memberships\b/i
      .test(sql),
    "Policy must target public.project_memberships.",
  );
});

Deno.test("API-HR.4 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.4 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.4 targets no second table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
  ...
  assertEquals(
    [...refs],
    ["project_memberships"],
    "Only public.project_memberships may be referenced.",
  );
});

Deno.test("API-HR.4 leaves pm_select_own and pm_select_admin untouched", async () => {
  const sql = await migrationSql();
  for (const name of ["pm_select_own", "pm_select_admin"]) {
...
Deno.test("API-HR.4 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.4 changes no grant, function, trigger, index or table definition", async () => {
  const sql = await migrationSql();
