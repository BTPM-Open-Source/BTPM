...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-22-ai-help-message-feedback-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.22 — public.ai_help_message_feedback OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.22 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.22");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.22 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.22");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.22");
});

Deno.test("API-HR.22 creates exactly one policy: api_e_oauth_read_containment on public.ai_help_message_feedback", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.ai_help_message_feedback\b/i
      .test(sql),
    "Policy must target public.ai_help_message_feedback.",
  );
});

Deno.test("API-HR.22 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.22 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.22 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
  ...
  assertEquals(
    ...
    "Only public.ai_help_message_feedback may be referenced.",
  );
});

Deno.test("API-HR.22 leaves the existing owner-only read policy untouched", async () => {
  const sql = await migrationSql();
  assert(
    !sql.includes("ai_help_fb_select_own"),
...
Deno.test("API-HR.22 leaves owner-authority logic untouched", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.22 leaves encryption helpers, triggers and feedback RPCs untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "trg_encrypt_ai_help_feedback_fields",
    "trg_encrypt_ai_help_feedback",
...
Deno.test("API-HR.22 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.22 changes no grant, function, trigger, index, view, enum, constraint or table definition", async () => {
  const sql = await migrationSql();
