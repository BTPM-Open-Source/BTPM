...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-39-tenant-integrations-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.39 — public.tenant_integrations OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.39 migration only. No live calls,
// no connection test, no credential access, no OAuth probe.
...
// Discovered during the API-HR.39 pre-check (live pg_policies).
const EXISTING_POLICIES = [
  "Tenant admins can view their tenant integrations",
  "Service role manages tenant_integrations",
];

// Discovered authority helpers in the live USING expression.
const AUTHORITY_HELPERS = ["is_tenant_admin", "is_platform_super_admin", "auth.uid()"];

// Discovered integration data / status / audit columns.
...
// Secret architecture, provider selection and integration runtime/test callers.
...
Deno.test("API-HR.39 migration exists, is unique and carries the marker", async () => {
...
Deno.test(`API-HR.39 creates exactly one policy api_e_oauth_read_containment on public.${TABLE}`, async () => {
...
Deno.test("API-HR.39 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.39 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.39 declares fail-closed base-table and duplicate-policy guards", async () => {
  const sql = await migrationSql();
  ...
  assert(
    /pg_policies/i.test(sql) &&
      /policyname\s*=\s*'api_e_oauth_read_containment'/i.test(sql),
    "Duplicate-policy guard required.",
  );
});

Deno.test("API-HR.39 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
...
Deno.test("API-HR.39 leaves existing policies and authority helpers untouched", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.39 references no integration data, status or audit column", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.39 references no secret, Vault, encryption, provider or runtime caller object", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.39 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.39 changes no grant, function, trigger, index, view, enum, constraint or table definition", async () => {
  const sql = await migrationSql();
