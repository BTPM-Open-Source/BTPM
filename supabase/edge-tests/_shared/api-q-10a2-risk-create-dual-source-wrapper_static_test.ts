// API-Q.10A2 — static contract guard for the canonical Risk-create dual-source
// wrapper bridge.
//
// Repository/static test only: it locates the committed API-Q.10A2 migration by
...
const MARKER = "API-Q.10A2 — Risk Create dual-source database wrapper bridge";
...
async function loadMigration(): Promise<{ name: string; text: string }> {
  ...
  assert(found.length >= 1, "expected at least one API-Q.10A2 migration");
...
const execBody = functionBody("api_e_private.execute_v1_create_risk");
const restBody = functionBody("public.api_v1_create_risk");
const mcpBody = functionBody("public.mcp_v1_create_risk");

Deno.test("API-Q.10A2: exactly three function surfaces are defined", () => {
  const created = sql.match(/CREATE OR REPLACE FUNCTION ([a-z_0-9.]+)/g) ?? [];
  assertEquals(created, [
    "CREATE OR REPLACE FUNCTION api_e_private.execute_v1_create_risk",
    "CREATE OR REPLACE FUNCTION public.api_v1_create_risk",
    "CREATE OR REPLACE FUNCTION public.mcp_v1_create_risk",
  ]);
});

Deno.test("API-Q.10A2: private executor has the exact fixed 14-argument signature", () => {
  const args = functionArgs("api_e_private.execute_v1_create_risk")
    .split(",")
...
Deno.test("API-Q.10A2: executor is SECURITY DEFINER with safe search_path", () => {
...
Deno.test("API-Q.10A2: execution source accepts exactly external_api | mcp", () => {
  assert(
  ...
  const literals = new Set(
    (execBody.match(/'(external_api|mcp|btpm_ui|admin_import|background_job|btpm_internal)'/g) ?? [])
      .map((m) => m.slice(1, -1)),
  );
  assertEquals(literals, new Set(["external_api", "mcp"]));
});

Deno.test("API-Q.10A2: external_api establishes API-E, mcp establishes MCP context", () => {
  assert(
  ...
  assertEquals(
    (execBody.match(/api_e_private\.authorize_and_establish_mcp\(/g) ?? []).length,
    1,
  );
  assert(/v_trusted IS NOT TRUE/.test(execBody));
});

Deno.test("API-Q.10A2: capability identity is exactly v1 / command / risks:create", () => {
  assert(/c_api_version\s+constant text := 'v1'/.test(execBody));
  assert(/c_capability_kind constant text := 'command'/.test(execBody));
  assert(/c_capability_key constant text := 'risks:create'/.test(execBody));
});

Deno.test("API-Q.10A2: target scope remains server-derived", () => {
...
Deno.test("API-Q.10A2: Project Connected App enablement remains enforced", () => {
...
Deno.test("API-Q.10A2: API-F idempotency claim/complete/fail remain present", () => {
  assert(
...
Deno.test("API-Q.10A2: exactly one hardcoded public.apply_risk_create call", () => {
  assertEquals(
    (execBody.match(/public\.apply_risk_create\(/g) ?? []).length,
    1,
  );
  assertEquals((sql.match(/public\.apply_risk_create\(/g) ?? []).length, 1);
});

Deno.test("API-Q.10A2: no dynamic RPC or function dispatch exists", () => {
...
Deno.test("API-Q.10A2: REST wrapper keeps its exact signature and fixes external_api", () => {
  const args = functionArgs("public.api_v1_create_risk")
    .split(",")
    .map((a) => a.trim().replace(/\s+/g, " "));
  assertEquals(args, [
    "_expected_oauth_client_id text",
  ...
  assert(
    /api_e_private\.execute_v1_create_risk\(\s*'external_api',/.test(restBody),
  );
  assert(!/'mcp'/.test(restBody));
});

Deno.test("API-Q.10A2: MCP wrapper mirrors the contract and fixes mcp", () => {
  assertEquals(
    functionArgs("public.mcp_v1_create_risk").replace(/\s+/g, " ").trim(),
    functionArgs("public.api_v1_create_risk").replace(/\s+/g, " ").trim(),
  );
  assert(/api_e_private\.execute_v1_create_risk\(\s*'mcp',/.test(mcpBody));
  assert(!/'external_api'/.test(mcpBody));
});

Deno.test("API-Q.10A2: neither public wrapper accepts an execution source", () => {
  for (const name of ["public.api_v1_create_risk", "public.mcp_v1_create_risk"]) {
    assert(
...
Deno.test("API-Q.10A2: private executor is unreachable by every app role", () => {
  for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
    assert(
      sql.includes(
        `REVOKE ALL ON FUNCTION api_e_private.execute_v1_create_risk${EXEC_ARGS} FROM ${role};`,
      ),
  ...
  assert(
    !new RegExp(
      `GRANT EXECUTE ON FUNCTION api_e_private\\.execute_v1_create_risk`,
    ).test(sql),
    "executor must never be granted",
  );
});

Deno.test("API-Q.10A2: public wrappers are delegated-user only", () => {
  for (const fn of ["public.api_v1_create_risk", "public.mcp_v1_create_risk"]) {
...
Deno.test("API-Q.10A2: Risk narrative never enters generic metadata", () => {
  // Narrative arguments are only forwarded to the canonical PMG command.
  const pmgStart = execBody.indexOf("public.apply_risk_create(");
  const pmgEnd = execBody.indexOf(");", pmgStart);
  const pmgCall = execBody.slice(pmgStart, pmgEnd);
  for (const narrative of ["_title", "_description", "_mitigation_plan"]) {
    const occurrences =
      (execBody.match(new RegExp(`(^|[^a-z0-9_])${narrative}([^a-z0-9_]|$)`, "g")) ?? [])
        .length;
    const inPmg =
      (pmgCall.match(new RegExp(`(^|[^a-z0-9_])${narrative}([^a-z0-9_]|$)`, "g")) ?? [])
        .length;
    assertEquals(
      occurrences,
      inPmg,
  ...
  assert(
    !/v_result := jsonb_build_object\([\s\S]*_description/.test(execBody),
  );
});

Deno.test("API-Q.10A2: execution source never enters payload, key or result", () => {
...
Deno.test("API-Q.10A2: no other canonical or security surface is touched", () => {
  for (const forbidden of [
    "apply_risk_update",
    "api_v1_update_risk",
    "CREATE OR REPLACE FUNCTION public.apply_risk_create",
...
Deno.test("API-Q.10A2: risks.create remains not_exposed and no MCP writer exists", async () => {
  const registry = await Deno.readTextFile(
    new URL("btpm-mcp/mcp/toolRegistry.ts", FUNCTIONS_DIR),
  );
  const at = registry.indexOf('operationId: "risks.create"');
  assert(at >= 0, "registry must still declare risks.create");
  // API-Q.10A5 exposed `risks.create`. What must still hold for this database
  // bridge step is that the registry entry keeps its canonical identity.
  ...
  for (const source of [factory, index]) {
    assert(
      ...
      "MCP runtime must not gain a Risk-create writer in this step",
    );
  }
});
