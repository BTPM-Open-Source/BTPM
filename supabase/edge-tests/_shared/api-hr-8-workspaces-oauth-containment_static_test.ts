...
const __BTPM_SRC_BASE__ = new URL('../../functions/_shared/api-hr-8-workspaces-oauth-containment_static_test.ts', import.meta.url).href;
// API-HR.8 — public.workspaces OAuth direct-read containment (static contract test).
//
// Structural inspection of the committed API-HR.8 migration only. No live calls.
...
async function migrationSql(): Promise<string> {
  const name = await findMigration("API-HR.8");
  return await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
}

Deno.test("API-HR.8 migration exists, is unique and carries the marker", async () => {
  const name = await findMigration("API-HR.8");
  assert(/^\d{14}_[0-9a-f-]+\.sql$/.test(name), `Unexpected name: ${name}`);
  const sql = await Deno.readTextFile(new URL(name, MIGRATIONS_DIR));
  assertStringIncludes(sql, "API-HR.8");
});

Deno.test("API-HR.8 creates exactly one policy: api_e_oauth_read_containment on public.workspaces", async () => {
  ...
  assert(
    /CREATE\s+POLICY\s+api_e_oauth_read_containment\s+ON\s+public\.workspaces\b/i.test(sql),
    "Policy must target public.workspaces.",
  );
});

Deno.test("API-HR.8 policy is restrictive SELECT for authenticated", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.8 USING expression is exactly the accepted containment", async () => {
  const sql = await migrationSql();
...
Deno.test("API-HR.8 targets no second business table", async () => {
  const sql = await migrationSql();
  const refs = new Set(
    [...sql.matchAll(/\bpublic\.([a-z_][a-z0-9_]*)/gi)].map((m) => m[1].toLowerCase()),
  );
  assertEquals([...refs], ["workspaces"], "Only public.workspaces may be referenced.");
});

Deno.test("API-HR.8 leaves ws_select_member_or_orgadmin, ws_insert_orgadmin and ws_update_admin untouched", async () => {
  const sql = await migrationSql();
  for (const name of ["ws_select_member_or_orgadmin", "ws_insert_orgadmin", "ws_update_admin"]) {
...
Deno.test("API-HR.8 leaves authority helper functions untouched", async () => {
  const sql = await migrationSql();
  for (const name of [
    "is_workspace_member",
    "is_org_admin",
...
Deno.test("API-HR.8 performs no DML or backfill", async () => {
  const sql = await migrationSql();
  for (const re of [
    /\bINSERT\s+INTO\b/i,
    /\bUPDATE\s+public\./i,
...
Deno.test("API-HR.8 changes no grant, function, trigger, index, view, enum or table definition", async () => {
  const sql = await migrationSql();
