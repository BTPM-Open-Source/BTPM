// API-K.6 — Dedicated transactional Blocker API wrappers.
//
// Repository static contract test. Locates the migration by its unique marker
// and asserts the frozen wrapper contract for public.api_v1_create_blocker and
// public.api_v1_update_blocker.

import { assert, assertEquals } from "https://deno.land/std@0.224.0/assert/mod.ts";

const MIGRATIONS_DIR = "supabase/migrations";
const MARKER = "API-K.6 — Dedicated DB wrappers api_v1_create_blocker and api_v1_update_blocker";
...
const CREATE_SQL = SQL.slice(
  SQL.indexOf("CREATE OR REPLACE FUNCTION public.api_v1_create_blocker("),
  SQL.indexOf("CREATE OR REPLACE FUNCTION public.api_v1_update_blocker("),
);
const UPDATE_SQL = SQL.slice(SQL.indexOf("CREATE OR REPLACE FUNCTION public.api_v1_update_blocker("));

Deno.test("API-K.6: exactly the two wrapper names and fixed typed signatures", () => {
  assertEquals([...SQL.matchAll(/CREATE OR REPLACE FUNCTION/g)].length, 2);
  assert(
    /CREATE OR REPLACE FUNCTION public\.api_v1_create_blocker\(\s*_expected_oauth_client_id text,\s*_target_type text,\s*_target_id uuid,\s*_title text,\s*_description text,\s*_severity text,\s*_status text,\s*_request_id text,\s*_correlation_id text,\s*_idempotency_key text,\s*_payload_hash text\s*\)/
      .test(SQL),
  );
  assert(
    /CREATE OR REPLACE FUNCTION public\.api_v1_update_blocker\(\s*_expected_oauth_client_id text,\s*_blocker_id uuid,\s*_expected_updated_at timestamptz,\s*_title text,\s*_description text,\s*_severity text,\s*_status text,\s*_request_id text,\s*_correlation_id text,\s*_idempotency_key text,\s*_payload_hash text\s*\)/
      .test(SQL),
  );
...
Deno.test("API-K.6: no caller scope, link or resolution inputs", () => {
  for (const sig of [
...
Deno.test("API-K.6: each wrapper has exactly one hardcoded capability identity", () => {
  assert(/c_capability_key constant text := 'blockers:create';/.test(CREATE_SQL));
  assert(/c_capability_key constant text := 'blockers:update';/.test(UPDATE_SQL));
  assertEquals(CREATE_SQL.includes("'blockers:update'"), false);
  assertEquals(UPDATE_SQL.includes("'blockers:create'"), false);
...
Deno.test("API-K.6: project connected app enablement runs before idempotency", () => {
  for (const part of [CREATE_SQL, UPDATE_SQL]) {
    ...
    for (
      const key of ["api_e.api_client_id", "api_e.tenant_id", "api_e.organization_id", "api_e.workspace_id"]
    ) {
...
Deno.test("API-K.6: API-F claim uses the exact fixed capability key", () => {
  assertEquals(
...
Deno.test("API-K.6: conflict, pending and replay branches never invoke PMG", () => {
  const cases: Array<[string, string]> = [
    [CREATE_SQL, "v_pmg := public.apply_blocker_create("],
    [UPDATE_SQL, "v_pmg := public.apply_blocker_update("],
  ];
  for (const [part, pmgCall] of cases) {
    ...
    assertEquals(between.includes("apply_blocker_create("), false);
    assertEquals(between.includes("apply_blocker_update("), false);
...
Deno.test("API-K.6: create calls only apply_blocker_create with empty link arrays", () => {
  assert(
    /v_pmg := public\.apply_blocker_create\(\s*v_target_type,\s*_target_id,\s*_title,\s*_description,\s*_severity,\s*_status,\s*'\[\]'::jsonb,\s*'\[\]'::jsonb,\s*_correlation_id,\s*_idempotency_key\s*\);/
      .test(CREATE_SQL),
  );
  assertEquals([...SQL.matchAll(/public\.apply_blocker_create\(/g)].length, 1);
  assertEquals(CREATE_SQL.includes("apply_blocker_update("), false);
});

Deno.test("API-K.6: update locks the Blocker, then reconstructs link state, never []", () => {
  assert(
    /FROM public\.blockers b\s*\n\s*WHERE b\.id = _blocker_id\s*\n\s*FOR UPDATE;/.test(UPDATE_SQL),
  );
  ...
  const pmgIdx = UPDATE_SQL.indexOf("v_pmg := public.apply_blocker_update(");
  ...
  // Locked attachment re-confirmation against the already-authorized scope.
  assert(UPDATE_SQL.includes("v_locked_target_type IS DISTINCT FROM v_row_target_type"));
  assert(UPDATE_SQL.includes("v_locked_workspace_id IS DISTINCT FROM v_workspace_id"));
  assert(UPDATE_SQL.includes("v_locked_organization_id IS DISTINCT FROM v_organization_id"));

  for (
    const clause of [
      "eul.owner_type = 'blocker'",
      "eul.owner_id = _blocker_id",
      "eul.link_role = 'related_person'",
      "'user_id', eul.user_id",
      "'stakeholder_id', eul.stakeholder_id",
      "ORDER BY eul.sort_order",
      "eol.owner_type = 'blocker'",
      "eol.owner_id = _blocker_id",
  ...
  assert(
    /v_pmg := public\.apply_blocker_update\(\s*_blocker_id,\s*_expected_updated_at,\s*_title,\s*_description,\s*_severity,\s*_status,\s*v_user_links,\s*v_object_links,\s*_correlation_id,\s*_idempotency_key\s*\);/
      .test(UPDATE_SQL),
  );
  assertEquals([...SQL.matchAll(/public\.apply_blocker_update\(/g)].length, 1);
  const call = UPDATE_SQL.slice(pmgIdx, UPDATE_SQL.indexOf(");", pmgIdx));
  assertEquals(call.includes("'[]'"), false);
});

Deno.test("API-K.6: wrappers perform no direct Blocker or link mutation", () => {
  ...
  assertEquals(/blockers\s+SET/i.test(SQL), false);
});

Deno.test("API-K.6: bounded results carry no narrative, resolvedBy or internal scope", () => {
...
Deno.test("API-K.6: stable safe failure codes and bounded outcome vocabulary", () => {
  assertEquals([...CREATE_SQL.matchAll(/fail_idempotency\(/g)].length, 2);
  assert(/fail_idempotency\(v_claim\.registry_id, 'stale_blocker'\)/.test(UPDATE_SQL));
  ...
  assertEquals(SQL.includes("stale_risk"), false);
  assertEquals(SQL.includes("current_updated_at"), false);
  assert(/RAISE EXCEPTION 'api_v1_create_blocker: unexpected canonical command status'/.test(SQL));
  assert(/RAISE EXCEPTION 'api_v1_update_blocker: unexpected canonical command status'/.test(SQL));
  ...
  // stale_blocker is the only explicit concurrency code exposed.
  assert(UPDATE_SQL.includes("'outcome', 'conflict', 'code', 'stale_blocker'"));
  assertEquals([...SQL.matchAll(/'code', '([a-z_]+)'/g)].map((m) => m[1]).filter((c) => c !== "stale_blocker").length, 0);
});

Deno.test("API-K.6: no dynamic SQL, generic dispatch or privileged impersonation", () => {
  for (
    const forbidden of [
      "format(",
      "quote_ident",
...
Deno.test("API-K.6: privileges are revoked from PUBLIC/anon and granted to authenticated only", () => {
  for (const fn of ["api_v1_create_blocker", "api_v1_update_blocker"]) {
...
Deno.test("API-K.6: protected canonical surfaces are not redefined", () => {
  for (
    const forbidden of [
