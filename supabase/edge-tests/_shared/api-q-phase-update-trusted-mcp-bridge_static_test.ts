// API-Q Phase Update Step 1 — static contract guard for the trusted MCP
// database bridge.
//
...
const MARKER = "API-Q Phase Update Step 1 — Trusted MCP Database Bridge";
...
async function loadMigration(): Promise<{ name: string; text: string }> {
  ...
  assert(found.length >= 1, "expected at least one Phase Update bridge migration");
...
const executor = (() => {
  const start = sql.indexOf(
    "CREATE OR REPLACE FUNCTION api_e_private.execute_v1_update_phase(",
  );
...
Deno.test("Phase Update bridge: exactly the four expected functions are (re)defined", () => {
  ...
  assertEquals(new Set(created), new Set([
    ...
    "api_e_private.execute_v1_update_phase",
    "public.api_v1_update_phase",
    "public.mcp_v1_update_phase",
  ]));
});

Deno.test("Phase Update bridge: canonical command keeps its exact signature", () => {
  assert(
    /CREATE OR REPLACE FUNCTION public\.apply_phase_update\(_phase_id uuid, _expected_updated_at timestamp with time zone, _name text, _description text DEFAULT NULL::text, _status pm_status DEFAULT NULL::pm_status, _phase_type phase_type DEFAULT NULL::phase_type, _correlation_id text DEFAULT NULL::text, _idempotency_key text DEFAULT NULL::text\)/
      .test(sql),
    "apply_phase_update signature must be unchanged",
  );
});

Deno.test("Phase Update bridge: canonical command accepts trusted external_api or mcp only", () => {
  assert(
...
Deno.test("Phase Update bridge: exact capability remains phases:update", () => {
  assert(
    sql.includes(
      "current_setting('api_e.capability_key', true),'')),''), '') <> 'phases:update'",
    ),
    "canonical command must pin phases:update",
  );
  assert(
    executor.includes("c_capability_key constant text := 'phases:update';"),
    "executor must pin phases:update",
  );
  assert(
...
Deno.test("Phase Update bridge: source channel remains server-derived only", () => {
  assert(
    sql.includes(
  ...
  assert(
    !/apply_phase_update\([^)]*_source_channel/.test(sql),
    "no source-channel parameter may be added to the canonical command",
  );
});

Deno.test("Phase Update bridge: private executor has the exact typed signature", () => {
  assert(
    /CREATE OR REPLACE FUNCTION api_e_private\.execute_v1_update_phase\(\s*_execution_source text,\s*_expected_oauth_client_id text,\s*_phase_id uuid,\s*_expected_updated_at timestamptz,\s*_name text,\s*_description text,\s*_status text,\s*_phase_type text,\s*_request_id text,\s*_correlation_id text,\s*_idempotency_key text,\s*_payload_hash text\s*\)\s*RETURNS jsonb/
      .test(sql),
...
Deno.test("Phase Update bridge: internal selector accepts external_api/mcp only", () => {
  assert(
...
Deno.test("Phase Update bridge: external_api uses authorize_and_establish, mcp uses the MCP helper", () => {
  assertEquals(
...
Deno.test("Phase Update bridge: Phase -> Project -> Workspace/Organization containment preserved", () => {
  assert(
    /SELECT ph\.project_id, ph\.workspace_id, ph\.organization_id\s*INTO v_row_project_id, v_row_workspace_id, v_row_organization_id/
      .test(executor),
  );
  assert(
  ...
  assert(
    ...
    "stored Phase scope must match the parent Project",
  );
});

Deno.test("Phase Update bridge: Connected App enablement precedes idempotency", () => {
...
Deno.test("Phase Update bridge: idempotency is claimed with the fixed capability key", () => {
  assert(
    executor.includes(
...
Deno.test("Phase Update bridge: execution branch locks and re-confirms Phase scope", () => {
  const claim = executor.indexOf("api_e_private.claim_idempotency(");
  const lock = executor.indexOf("FOR UPDATE");
  const apply = executor.indexOf("public.apply_phase_update(");
  ...
  assert(
    ...
    "scope drift must fail the idempotency claim as not_authorized",
  );
});

Deno.test("Phase Update bridge: exactly one canonical command call in the shared executor", () => {
  assertEquals(
    (executor.match(/public\.apply_phase_update\(/g) ?? []).length,
    1,
  );
});

Deno.test("Phase Update bridge: caller expected_updated_at is forwarded unchanged", () => {
  assert(
    /public\.apply_phase_update\(\s*_phase_id,\s*_expected_updated_at,\s*_name,\s*_description,\s*v_status,\s*v_phase_type,\s*_correlation_id,\s*_idempotency_key\s*\)/
      .test(executor),
    "the caller-supplied concurrency token must be forwarded verbatim",
  );
  // No read-before-write refresh and no retry.
  assert(
    !/_expected_updated_at\s*:=/.test(executor),
    "expected_updated_at must never be reassigned",
  );
  assert(
    !/ph\.updated_at/.test(executor) && !/updated_at\s+INTO/.test(executor),
...
Deno.test("Phase Update bridge: stale conflict remains bounded and non-leaking", () => {
  assert(
    executor.includes(
  ...
  const staleReturns =
    (executor.match(
      /jsonb_build_object\('ok', false, 'outcome', 'conflict', 'code', 'stale_phase'\)/g,
    ) ?? []).length;
...
Deno.test("Phase Update bridge: applied / no_change / replayed remain represented", () => {
...
Deno.test("Phase Update bridge: no Phase narrative is returned or persisted", () => {
...
Deno.test("Phase Update bridge: public wrappers hardcode their fixed source", () => {
  assert(
    /CREATE OR REPLACE FUNCTION public\.api_v1_update_phase\([\s\S]*?RETURN api_e_private\.execute_v1_update_phase\(\s*'external_api',/
      .test(sql),
    "REST wrapper must hardcode external_api",
  );
  assert(
    /CREATE OR REPLACE FUNCTION public\.mcp_v1_update_phase\([\s\S]*?RETURN api_e_private\.execute_v1_update_phase\(\s*'mcp',/
      .test(sql),
    "MCP wrapper must hardcode mcp",
  );
  assert(
    !/_execution_source text/.test(
      sql.slice(sql.indexOf("CREATE OR REPLACE FUNCTION public.api_v1_update_phase(")),
    ),
    "no public wrapper may accept a source parameter",
  );
});

Deno.test("Phase Update bridge: private executor is revoked from every application role", () => {
  const args =
    "\\(text, text, uuid, timestamptz, text, text, text, text, text, text, text, text\\)";
  for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
    assert(
      new RegExp(
        `REVOKE ALL ON FUNCTION api_e_private\\.execute_v1_update_phase${args} FROM ${role};`,
      ).test(sql),
      `private executor EXECUTE must be revoked from ${role}`,
    );
  }
  assert(
    !/GRANT[^;]*api_e_private\.execute_v1_update_phase/.test(sql),
    "the private executor must never be granted",
  );
});

Deno.test("Phase Update bridge: MCP wrapper ACL matches the accepted posture", () => {
  const args =
    "\\(text, uuid, timestamptz, text, text, text, text, text, text, text, text\\)";
  for (const role of ["PUBLIC", "anon", "service_role"]) {
    assert(
      new RegExp(
        `REVOKE ALL ON FUNCTION public\\.mcp_v1_update_phase${args} FROM ${role};`,
      ).test(sql),
      `MCP wrapper must revoke ${role}`,
    );
  }
  assert(
    new RegExp(
      `GRANT EXECUTE ON FUNCTION public\\.mcp_v1_update_phase${args} TO authenticated;`,
    ).test(sql),
  );
  assert(
    new RegExp(
      `GRANT EXECUTE ON FUNCTION public\\.api_v1_update_phase${args} TO authenticated;`,
    ).test(sql),
  );
  assert(
    !/GRANT[^;]*(mcp_v1_update_phase|api_v1_update_phase)[^;]*TO (PUBLIC|anon|service_role)/
      .test(sql),
  );
});

Deno.test("Phase Update bridge: no planning, reorder or Project widening is introduced", () => {
  for (const forbidden of [
    "apply_phase_planning",
    "apply_phase_reorder",
    ...
    assert(
      ...
      `Phase Update must remain metadata-only: ${forbidden} must not appear`,
    );
  }
});

Deno.test("Phase Update bridge: no MCP registry or runtime exposure is introduced", async () => {
  ...
  // Phase Update Step 4 exposed and wired `phases.update`. The durable bridge
  // invariant is that neither the registry nor the MCP runtime ever names the
  // database wrapper directly: only the accepted caller-bound adapter may.
  for (const source of [registry, index]) {
    assert(
      !source.includes("mcp_v1_update_phase"),
      "no MCP registry or runtime module may name the Phase Update wrapper",
    );
  }
  const phasesUpdate = registry.match(
    /operationId: "phases\.update"[\s\S]{0,600}?\}\)/,
  );
  assert(phasesUpdate, "phases.update must remain declared in the registry");
  assert(
    phasesUpdate[0].includes('exposure: "exposed"'),
    "phases.update remains canonically declared and exposed",
  );
