-- BTPM OSS baseline: current Phase reorder private execution bridge.
...
BEGIN
  ...
  IF v_claim.decision IS NULL THEN
    RAISE EXCEPTION 'execute_v1_reorder_phases: no idempotency claim decision' USING ERRCODE = 'XX000';
  END IF;
  ...
  ELSIF v_claim.decision = 'replay' THEN
    IF v_claim.registry_state = 'completed' THEN
      IF v_claim.canonical_result IS NULL OR jsonb_typeof(v_claim.canonical_result) <> 'object' THEN
        RAISE EXCEPTION 'execute_v1_reorder_phases: invalid stored canonical result' USING ERRCODE = 'XX000';
      END IF;
      RETURN v_claim.canonical_result || jsonb_build_object('outcome', 'replayed');
    ELSIF v_claim.registry_state = 'failed' THEN
      IF v_claim.failure_code = 'stale_phase_order' THEN
        RETURN jsonb_build_object('ok', false, 'outcome', 'conflict', 'code', 'stale_phase_order');
      ELSIF v_claim.failure_code = 'not_authorized' THEN
        RETURN jsonb_build_object('ok', false, 'outcome', 'not_authorized');
      ELSIF v_claim.failure_code = 'invalid' THEN
        RETURN jsonb_build_object('ok', false, 'outcome', 'invalid');
      END IF;
      RAISE EXCEPTION 'execute_v1_reorder_phases: unknown persisted failure code' USING ERRCODE = 'XX000';
    END IF;
    RAISE EXCEPTION 'execute_v1_reorder_phases: unexpected replay state' USING ERRCODE = 'XX000';
  ELSIF v_claim.decision <> 'execute' THEN
    RAISE EXCEPTION 'execute_v1_reorder_phases: unexpected idempotency decision' USING ERRCODE = 'XX000';
  END IF;

  v_pmg := public.reorder_phases(_project_id, _rows, _correlation_id, _idempotency_key);
  ...
  IF v_pmg_status IN ('applied','no_change') THEN
    IF jsonb_typeof(v_data -> 'submitted_count') <> 'number'
       ...
       OR jsonb_typeof(v_data -> 'ordered_phases') <> 'array'
    THEN
      RAISE EXCEPTION 'execute_v1_reorder_phases: inconsistent canonical result' USING ERRCODE = 'XX000';
    END IF;

    BEGIN
      SELECT coalesce(jsonb_agg(jsonb_build_object(
               'phaseId', (elem ->> 'id')::uuid,
               'sortOrder', (elem -> 'sort_order'),
               'updatedAt', (elem ->> 'updated_at')
             )), '[]'::jsonb)
        INTO v_ordered
        FROM jsonb_array_elements(v_data -> 'ordered_phases') AS elem;
    EXCEPTION WHEN OTHERS THEN
      RAISE EXCEPTION 'execute_v1_reorder_phases: malformed canonical ordering' USING ERRCODE = 'XX000';
    END;

    v_result := jsonb_build_object(
      'ok', true, 'outcome', v_pmg_status, 'projectId', v_project_id,
      'submittedCount', (v_data -> 'submitted_count'),
      'changedCount', (v_data -> 'changed_count'),
      'orderedPhases', v_ordered
    );
    PERFORM api_e_private.complete_idempotency(v_claim.registry_id, v_result);
    RETURN v_result;
  ELSIF v_pmg_status = 'conflict' THEN
    IF v_conflict IS NULL OR (v_conflict ->> 'code') IS DISTINCT FROM 'stale_phase_order' THEN
      RAISE EXCEPTION 'execute_v1_reorder_phases: unexpected conflict payload' USING ERRCODE = 'XX000';
    END IF;
    v_stale := '[]'::jsonb;
    IF jsonb_typeof(v_conflict -> 'stale_ids') = 'array' THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE EXCEPTION 'execute_v1_reorder_phases: malformed conflict payload' USING ERRCODE = 'XX000';
      END;
    END IF;
    PERFORM api_e_private.fail_idempotency(v_claim.registry_id, 'stale_phase_order');
    RETURN jsonb_build_object(
      'ok', false, 'outcome', 'conflict', 'code', 'stale_phase_order',
      'projectId', v_project_id, 'stalePhaseIds', v_stale
    );
  ...
  RAISE EXCEPTION 'execute_v1_reorder_phases: unexpected canonical command status' USING ERRCODE = 'XX000';
END;
$function$;

REVOKE ALL ON FUNCTION api_e_private.execute_v1_reorder_phases(text, text, uuid, jsonb, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION api_e_private.execute_v1_reorder_phases(text, text, uuid, jsonb, text, text, text, text) FROM anon;
REVOKE ALL ON FUNCTION api_e_private.execute_v1_reorder_phases(text, text, uuid, jsonb, text, text, text, text) FROM authenticated;
REVOKE ALL ON FUNCTION api_e_private.execute_v1_reorder_phases(text, text, uuid, jsonb, text, text, text, text) FROM service_role;
